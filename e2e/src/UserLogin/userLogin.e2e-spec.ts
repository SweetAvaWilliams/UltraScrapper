...
describe('when Ultra App initialize ', () => {
  ...
  describe('when a user tries to log into UltraApp ', () => {
    ...
    it('using an existing email but invalid password an error message should be displayed', () => {
      page.performSignIn('yghor.kerscher@toptal.com', 'fakepassword2');
  ...
  describe('and a user tries to log into UltraApp ', () => {
    beforeEach(async() => {
      await sqlCmd.ExecuteQuery(`update ultra.users set IsActive = 0 where email = \'ben.carey@toptal.com\';`);
    });
    ...
    afterEach(async() => {
      await sqlCmd.ExecuteQuery(`update ultra.users set IsActive = 1 where email = \'ben.carey@toptal.com\';`);
    });
  });
});
